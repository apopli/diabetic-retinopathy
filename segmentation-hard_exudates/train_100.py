...
def BCE_(y_pred, y_true):
    # weight ratio = 3:1
    # 3-1=2
    class_weights = tf.constant([2],dtype=tf.float32)
...
def main(flags):
    ...
    BCE_op = BCE_(pred, y)
    BCE_op = tf.Print(BCE_op, [BCE_op])
    tf.summary.scalar("BCE", BCE_op)
        ...
        try:
            ...
            for epoch in range(flags.epochs):
                ...
                for step in range(0, n_train, flags.batch_size):
                    ...
                    #                y: y_batch,
                    #                mode: True})
                    _, step_bce, step_summary, global_step_value = sess.run(
                        [train_op, BCE_op, summary_op, global_step],
                        feed_dict={X: X_batch,
                                   y: y_batch,
                                   mode: True})
                ...
                #     #                y: y_test,
                #     #                mode: False})
                #     step_bce, step_summary = sess.run(
                #         [BCE_op, summary_op],
                #         feed_dict={X: X_test,
                #                    y: y_test,
                #                    mode: False})
                ...
                #     total_bce += step_bce * X_test.shape[0]
