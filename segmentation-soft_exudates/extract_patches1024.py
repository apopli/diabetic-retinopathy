    ...
    for i in range(5):
    	for j in range(8):
            ...
            if (np.sum(np.array(im_crop)) < 100):
                negative_patches.append(output_dir_mask+outfile+"_1024p"+str(patch_id)+extension)
            else:
                positive_count += 1
            im_crop = resizeimage.resize_cover(im_crop, [512, 512])
            imd_crop = resizeimage.resize_cover(imd_crop, [512, 512])
            im_crop.save(output_dir_mask+outfile+"_1024p"+str(patch_id)+extension,"JPEG",quality=100)
            imd_crop.save(output_dir_data+outfile+"_1024p"+str(patch_id)+extension,"JPEG",quality=100)
